...
class MultiImageObsEncoder(nn.Module):
    def __init__(self, vision_encoder, image_keys):
        super(MultiImageObsEncoder, self).__init__()
        self.vision_encoder = vision_encoder
        self.image_keys = image_keys

    def forward(self, images_dict):
        batch_size, obs_horizon = images_dict[self.image_keys[0]].shape[:2]

        img_features = []
        for key in self.image_keys:
            images = images_dict[key]
            features = self.vision_encoder(images.flatten(end_dim=1))
            features = features.reshape(batch_size, obs_horizon, -1)
